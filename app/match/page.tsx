...
export default function MatchPage() {
  ...
  const [matches, setMatches] = useState<RivaloMatch[]>([]);
  ...
  async function loadData(uid: string) {
    ...
    try {
      ...
      const loadedGroups: RivaloGroup[] = groupsSnapshot.docs.map((docSnap) => ({
        id: docSnap.id,
      ...
      const loadedMatches: RivaloMatch[] = matchesSnapshot.docs.map((docSnap) => ({
        id: docSnap.id,
        ...(docSnap.data() as Omit<RivaloMatch, "id">),
      }));
  ...
  async function createMatch(e: React.FormEvent) {
    ...
    try {
      ...
      setMode("amichevole");
      setSlots("10");
      setMessage("Partita creata. Ora è visibile nella lista delle tue partite.");
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-5 py-8">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1.05fr_.95fr]">
          <section className="rounded-[2rem] border border-white/10 bg-[#071126]/75 p-7 shadow-2xl backdrop-blur">
            ...
            <form onSubmit={createMatch} className="space-y-5">
              <Field label="Gruppo collegato">
                <select value={groupId} onChange={(e) => setGroupId(e.target.value)} className="w-full bg-transparent outline-none" required>
              ...
              <button type="submit" disabled={saving || !groupId} className="group flex w-full items-center justify-center gap-3 rounded-2xl bg-gradient-to-r from-cyan-400 to-fuchsia-500 px-6 py-4 font-black shadow-[0_0_30px_rgba(34,211,238,.18)] disabled:opacity-60">
          ...
          <section className="space-y-6">
            ...
            <InfoCard icon={<Trophy size={28} />} title="Ranking" text="Ogni match può aggiornare classifiche." />
            <InfoCard icon={<ShieldCheck size={28} />} title="FairPlay" text="Conferma risultati e validazione gruppo." />
            <InfoCard icon={<Clock size={28} />} title="Reminder post-partita" text="Dopo data e ora, Rivalo potrà ricordare di inserire risultato e statistiche." />
          </section>
        </div>
